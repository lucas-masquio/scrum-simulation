...
import { fetchDados, saveDados } from "./api";

const AUTO_SAVE_DELAY_MS = 1500; // espera essa pausa na digitação antes de salvar sozinho
...
function App() {
  ...
  const [saveStatus, setSaveStatus] = useState("idle"); // idle | saving | saved | erro
  ...
  // Carrega o último estado salvo no backend assim que o painel abre.
  // Segue o mesmo princípio da lista de alunos: se não existir nada salvo ainda,
  // cai para os dados padrão (buildInitialData) em vez de travar a tela.
  useEffect(() => {
    let ativo = true;
    (async () => {
      try {
        const dadosSalvos = await fetchDados();
        if (ativo && dadosSalvos) {
          setData(ensureLoadedData(dadosSalvos));
          setFileName("dados salvos no servidor");
        }
      } catch (error) {
        console.error("Não foi possível carregar dados do servidor:", error);
      } finally {
  ...
  // Auto-save: sempre que "data" muda, espera uma pequena pausa e salva sozinho no backend.
  // Não roda na primeira renderização nem enquanto o carregamento inicial não terminou,
  // para não sobrescrever dados salvos com os dados padrão por engano.
  useEffect(() => {
  ...
  // Salvamento manual: chamado pelo botão "Salvar agora". Vai direto pro backend.
  async function saveData() {
    ...
    try {
      ...
      console.error("Falha ao salvar manualmente:", error);
      setSaveStatus("erro");
  ...
  if (loadingInicial) {
    return (
      <div className="app-shell app-loading">
        <p>Carregando dados salvos no servidor...</p>
      </div>
    );
  }
...
function SaveStatusLabel({ status }) {
  const labels = {
    ...
    erro: "falha ao salvar — verifique o backend",
  };
